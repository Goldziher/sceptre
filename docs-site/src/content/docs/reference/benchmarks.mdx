...
| **sceptre** (warm/batch) | **1.07** (~2.3×) | **6,654.8** (~3.1× lower) | 0.570 | 0.361 |
...
<Aside type="note">
  The regression gate (`--assert`) enforces floors on warm speedup, peak-RSS ratio, and labeled
  token-F1 against EasyOCR — the exact thresholds live in the harness, not here, since peak-RSS
  parity narrows as page size grows. Run manually via `workflow_dispatch` in CI, not on every PR
  (the reference run needs torch, easyocr, and the full model set).
</Aside>
