...
| **sceptre** (warm/batch) | **1.07** (~2.3×) | **6,654.8** (~3.8× lower, per-image median) | 0.570 | 0.361 |
...
EasyOCR does. Peak RSS is dominated by the CRAFT detector both engines share, so its ratio narrows
toward parity as page size grows — throughput is the more stable difference between the two.
...
[ADR 0021](https://github.com/xberg-io/sceptre/blob/main/adrs/0021-benchmark-methodology-and-gate.md).
...
[ADR 0030](https://github.com/xberg-io/sceptre/blob/main/adrs/0030-published-benchmark-artifact-and-drift-gate.md).
...
[`crates/sceptre/tests/data/golden/`](https://github.com/xberg-io/sceptre/tree/main/crates/sceptre/tests/data/golden).

<Aside type="note">
  The regression gate (`--assert`) enforces a floor on warm speedup against EasyOCR, and a ceiling
  on sceptre's *own* peak RSS relative to the published baseline for the same host — a cross-engine
  memory ratio makes a poor gate, since peak RSS is dominated by the CRAFT detector both engines
  share ([ADR 0042](https://github.com/xberg-io/sceptre/blob/main/adrs/0042-host-scoped-benchmark-floors.md)).
  Labeled quality is held by per-image guardrail contracts. The exact thresholds live in the
  harness, not here. Run manually via `workflow_dispatch` in CI, not on every PR (the reference
  run needs torch, easyocr, and the full model set).
</Aside>
