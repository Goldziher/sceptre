...
A golden fixture is a claim about two runtimes, so each one records which runtimes
produced it in a `metadata` block with an independent half per side. Fixtures are
...
`metadata` is provenance only — the parity tests ignore it, so an older fixture without
the block still loads. `onnxruntime_version` is `null` because the runtime version is not
exposed through the `ModelBackend` seam; `git_commit` is `null` when the generator ran
