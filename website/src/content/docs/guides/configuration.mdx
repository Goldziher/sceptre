...
| `accelerator` | `Accelerator::Cpu` | Hardware the backend runs the graph on: `Cpu`, `Auto`, `CoreMl`, `DirectMl`, or `Cuda`. Only `Backend::Ort` can run on a non-CPU accelerator; anything else is rejected by validation. See [Backends](/concepts/backends/#accelerator-selection). |
...
| `registry_owner` | `None` | Override for the Hugging Face registry owner. `None` uses the first-party `sceptre-ocr` org; only the owner segment of each model repo id changes. Library-only — no CLI flag. |
...
requires `ort` (via `ort-bundled` or `ort-dynamic`). A non-CPU `accelerator` additionally requires
the matching feature — `ort-coreml`, `ort-directml`, or `ort-cuda` — and an ONNX Runtime build that
