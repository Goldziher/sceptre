...
<Aside type="note">
  ...
  and labeled token-F1 within 0.05 of EasyOCR — run via `workflow_dispatch` and a weekly schedule in
  CI, not on every PR (the reference run needs torch, easyocr, and the full model set).
</Aside>
